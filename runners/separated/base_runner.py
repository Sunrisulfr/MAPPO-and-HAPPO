...
class Runner(object):
    def __init__(self, config):
        ...
        for agent_id in range(self.num_agents):
            ...
            bu = SeparatedReplayBuffer(self.all_args,
                                       self.envs.observation_space[agent_id],
                                       share_observation_space,
                                       self.envs.action_space[agent_id], self.num_agents)
    ...
    def train(self):
        ...
        old_factor = np.ones((self.episode_length, self.n_rollout_threads, action_dim), dtype=np.float32)

        for agent_id in torch.randperm(self.num_agents):
            self.trainer[agent_id].prep_training()
            self.buffer[agent_id].update_factor(factor)
            self.buffer[agent_id].update_old_factor(old_factor)
            ...
            if self.all_args.algorithm_name == "hatrpo":
                ...
                old_actions_logprob, _ =self.trainer[agent_id].policy.actor.evaluate_actions(self.buffer[agent_id].obs[:-1].reshape(-1, *self.buffer[agent_id].obs.shape[2:]),
                                                            ...
                                                            self.buffer[agent_id].actions.reshape(-1, *self.buffer[agent_id].actions.shape[2:]),
                                                            self.buffer[agent_id].masks[:-1].reshape(-1, *self.buffer[agent_id].masks.shape[2:]),
                                                            self.buffer[agent_id].action_log_probs.reshape(-1, *self.buffer[agent_id].action_log_probs.shape[2:]),
                                                            available_actions,
                                                            self.buffer[agent_id].active_masks[:-1].reshape(-1, *self.buffer[agent_id].active_masks.shape[2:]))
            ...
            # old_factor = old_factor*_t2n(old_action_log_probs_batch.reshape(self.episode_length,self.n_rollout_threads,action_dim))
            # factor = factor*_t2n(new_action_log_probs_batch.reshape(self.episode_length,self.n_rollout_threads,action_dim))
