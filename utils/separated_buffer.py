...
class SeparatedReplayBuffer(object):
    def __init__(self, args, obs_space, share_obs_space, act_space, num_agents):
        ...
        self.other_actions = np.zeros((self.episode_length, self.n_rollout_threads, act_shape*(self.num_agents-1)), dtype=np.float32)
    ...
    def update_old_factor(self, old_factor):
        self.old_factor = old_factor.copy()

    def insert(self, share_obs, obs, rnn_states, rnn_states_critic, actions, other_actions, action_log_probs,
        ...
        self.other_actions[self.step] = other_actions.copy().reshape(other_actions.shape[0], -1)
        ...
        if self._use_proper_time_limits:
            ...
            if self._use_gae:
                ...
                for step in reversed(range(self.rewards.shape[0])):
                    if self._use_popart or self._use_valuenorm:
                        log_reward = self.update_surprise_rewards(step, means, stds)
                        delta = self.rewards[step]  + log_reward + self.gamma * value_normalizer.denormalize(self.value_preds[step + 1]) * self.masks[step + 1] - value_normalizer.denormalize(self.value_preds[step])
                        gae = delta + self.gamma * self.gae_lambda * self.masks[step + 1] * gae
                        self.returns[step] = gae + value_normalizer.denormalize(self.value_preds[step])
                    else:
                        log_reward = self.update_surprise_rewards(step, means, stds)
                        delta = self.rewards[step] + log_reward + self.gamma * self.value_preds[step + 1] * self.masks[step + 1] - self.value_preds[step]
                        gae = delta + self.gamma * self.gae_lambda * self.masks[step + 1] * gae
                        self.returns[step] = gae + self.value_preds[step]
            else:
                self.returns[-1] = next_value
                for step in reversed(range(self.rewards.shape[0])):
                    log_reward = self.update_surprise_rewards(step, means, stds)
                    self.returns[step] = self.returns[step + 1] * self.gamma * self.masks[step + 1] + self.rewards[step] + log_reward
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        if self.old_factor is not None:
            # factor = self.factor.reshape(-1,1)
            old_factor = self.old_factor.reshape(-1, self.old_factor.shape[-1])
        ...
        for indices in sampler:
            ...
            if self.factor is None:
                ...
                old_factor_batch = old_factor[indices]
                yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch, factor_batch, old_factor_batch
    ...
    def naive_recurrent_generator(self, advantages, num_mini_batch):
        ...
        for start_ind in range(0, n_rollout_threads, num_envs_per_batch):
            ...
            for offset in range(num_envs_per_batch):
                ...
                if self.old_factor is not None:
                    old_factor_batch.append(self.old_factor[:,ind])
            ...
            if self.old_factor is not None:
                old_factor_batch=np.stack(old_factor_batch,1)
            ...
            if self.old_factor is not None:
                old_factor_batch=_flatten(T,N,old_factor_batch)
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        if self.old_factor is not None:
            old_factor = _cast(self.old_factor)
        ...
        for indices in sampler:
            ...
            for index in indices:
                ...
                if self.old_factor is not None:
                    old_factor_batch.append(old_factor[ind:ind+data_chunk_length])
            ...
            if self.old_factor is not None:
                old_factor_batch = np.stack(old_factor_batch)
            ...
            if self.old_factor is not None:
                old_factor_batch = _flatten(L, N, old_factor_batch)
